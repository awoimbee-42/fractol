...
/*   Updated: 2018/12/31 04:21:43 by awoimbee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_line_higrad(t_pixel p0, t_pixel p1, int *img, int win_w)
{
	...
	{
		img[(p0.im_y * win_w + p0.re_x)] = 0xFF;
		if (error > 0)
		{
			p0.re_x += way_x;
			error -= 2 * delta.im_y;
...
void kochCurve(t_complex p1,t_complex p2,int times, t_env *env){
	...
	if (times > 0)
	{
		...
		p4 = (t_complex){p3.re + (p5.re - p3.re)*cos(theta) + (p5.im - p3.im)*sin(theta),p3.im - (p5.re - p3.re)*sin(theta) + (p5.im - p3.im)*cos(theta)};
	...
	{
		draw_line((t_pixel){p1.re, p1.im}, (t_pixel){p2.re, p2.im}, env->mlx->img.px, env->res.w);
	}
}
...
void		*draw_koch(void *data)
{
	...
	env = data;
	p_start = (t_c){0, env->res.h / 2};
	p_end = (t_c){env->res.w, env->res.h / 2};
	kochCurve(p_start, p_end, 5, env);
	return (NULL);
}